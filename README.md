...
El footer muestra el stack de **esta** página (HTML · CSS · JS) con tooltip, más iconos fork/repo. El de fork apunta a `/fork` y copia este repo a tu cuenta (requiere iniciar sesión). También puedes usar el botón **Fork** en la página del repo.

Para publicar tu propia copia en GitHub Pages (mismo modelo `main` = fuente, `gh-pages` = build):

1. **Fork** este repo (botón de GitHub o icono del footer; o clónalo y apunta `origin` a tu fork).
